...
- プロジェクト概要  
顧客のAI研究部では、MLパイプライン構築やスクリプト、コンテナイメージのバージョン管理をすべて手動で実施しており、処理実行時間の短縮とスクリプト管理効率の改善が課題となっていた。  
...
- 担当業務
  - 技術選定  
他部署におけるAWS利用実績とその評判の高さから、顧客はAWSを利用したML開発に興味を持っていた。そのため、クラウドはAWS、サービスは包括的なML開発をサポートしているSageMaker Studioを採択、クラウドに不慣れな顧客の技術選択をサポートした。

  - 既存コードの分析とマネージドサービスへの移行  
顧客のエンジニアからMLに関する知見のサポートを受けつつ、SageMaker Studioに移行予定の既存のMLスクリプトを分析。顧客に代わってAWSのサービスにマネージド可能なもの・不可なものを切り分けた。具体的には、ML処理の中間生成物のI/OをS3に変更。また、前処理・学習・モデル評価の各処理を再利用可能な単位に切り分け、SageMakerのカスタムコンテナで実行可能となるように整備。  
現在は、SageMaker Pipelineを用いて前処理・学習とモデル評価をパイプラインにまとめ、SageMaker Experimentsによって一連の処理フローとそのパラメータ推移を「実験」という単位で管理できるように変更作業中。  
これにより、顧客はビジネスロジック（前処理・学習・モデル評価）のみに集中可能となった。また、8時間以上かかっていた一連の処理実行時間を数時間程度に短縮できる見込み。

  - ML開発リソースの権限周りの整備  
顧客のAWS利用方法をヒアリングし、ML開発に必要となるAWS IAMの権限周りを整備。ペイメント管理責任のあるユーザーが、必要な時のみ支払い情報にアクセスできるようにIAMスイッチロール機能を構築した。越権操作実行の発生を抑制することにより、顧客の安全なAWSリソース運用管理の実現に貢献した。

  - ML開発環境のIaC化  
AWSへの移行が成功した後は、他部署でも簡単に同一のMLOps統合開発環境を構築できるような運用体制を構築したいという要望を受けた。  
AWSコンソールのUIは不定期に変更されるため、画面キャプチャを用いた作業手順は不採用とした。代わりに、再現性の観点から開発インフラをCloudFormationでIaC管理し、その実行手順を整備。インフラコードはリソースのライフサイクルを考慮してML・IAM権限・ユーザー情報の3種に分け、スタックを分割して管理できるようにした。  
これにより、顧客のスムーズな開発環境の移行とインフラの再現が可能となった。

  - 顧客のMLエンジニアへのAWS技術教育  
顧客による中長期的なAWSの安定利用を実現するため、顧客のエンジニアに向けたAWS技術のレクチャーを実施した。MLやその他のAWS各種ハンズオンを利用し、詳細なアーキテクチャ図を通してAWSリソースの関連性を確認できるように資料を作成し、顧客のエンジニアのAWS理解をサポートした。

### 社内システムのクラウドセキュリティ改善計画立案（2022年4月〜2022年9月）
- プロジェクト概要  
AWSの利用増大と用途の多様化に伴い、顧客のシステムでは膨大な数のセキュリティアラームが発生していた。インシデント発生予防と監視運用体制の確立が急務となったため、開発メンバーとして調査とKPIの設定、及びセキュリティ改善計画の作成と提案を実施した。  
...
また、定量的に現状を把握するため、Trusted Advisorの組織ビュー情報を集計し、アカウント別のセキュリティアラーム項目出力数を算出した。これによって、100以上存在する各アカウント上のシステムのセキュリティレベルにはバラツキがあること、古いリソースの利用によるアラーム発生がある程度の割合で存在していること、特定のセキュリティアラームが一部のアカウントに集中していることなど、戦略立案のための重要な情報を把握することができた。

  - 戦略立案  
組織としてのクラウド利用ガイドラインは必須のため、その作成方法を模索した。AWSにはWell-Architectedセキュリティ要領に対応したベストプラクティスが存在するため、それに準拠することでゼロからガイドラインを作らずにセキュリティを高めることが可能である。ベストプラクティスは数が多いため「全アカウントへの適用可能性」「既存システムへの影響範囲の低さ」「現状体制での運用可能性」という選択軸を設け、ベストプラクティスの取り捨て選択を実施することを提案。しかしながら、ベストプラクティスの数の多さ、選択軸に該当しないベストプラクティスが存在すること、それによる選択軸そのものの選択の困難化、KPI達成期間内の現実的な実施可能性がチーム内で考慮された結果、この戦略案は不採用となった。  
次にTrusted Advisorを利用し、セキュリティ項目別の施策を実施してくことを検討。Trusted Advisorがセキュリティの専門家が選定した基本的なベストプラクティスを提案するAWSの公式ツールであること、セキュリティアラームを出力している項目と数は集計で把握していること、セキュリティ項目が限られており選択軸を設定して優先順位をつけることが可能であることからこれを提案。多少回り道をすることとなったが、顧客を説得できる現実的な戦略案として採択された。

  - 戦術立案  
Trusted Advisorの各セキュリティ項目ごとに具体的な実施計画を立ててKPIを設定。また、各施策の優先順位付けに客観的な説得力を持たせるため、CVSS(共通脆弱性評価システム)のスコア値を採択。これをもとに、セキュリティ施策の実行計画（マイルストーン）を作成した。
これにより、施策実施スケジュールと予算獲得のための道筋をたてることが可能となった。
