...
顧客のMLエンジニアからMLに関する知見のサポートを受け、AWS SageMaker Studioに移行予定の既存コードを分析。AWSのサービスにマネージド可能なもの・不可なものを切り分けた。具体的には、ML処理の中間生成物のインプット・アウトプットをS3に変更。また、前処理・トレーニング・推論の各処理を再利用可能な単位に切り分け、SageMakerのカスタムコンテナで実行可能となるように整備した。

  - ML開発リソースの権限周りの整備  
顧客からのヒアリングにより、AWSのML開発に必要となるIAMの権限周りを整備。ペイメント管理責任のあるユーザーが、必要な時のみ支払い情報にアクセスできるようにスイッチロール機能を構築した。本来は不可能な操作が誤って実行されることを防ぐことにより、システムのセキュリティ向上に貢献した。

  - ML開発環境のIaC化  
開発で必要となるインフラをCloudFormationでIaC化。リソースのライフサイクルを考慮してML・IAM権限・ユーザー情報の3種に分け、スタックを分割して管理できるように整備した。これにより、他部署のAWSアカウントで同一のML開発環境をすばやく簡単に再現可能となった。
